...
RTK_UART2 checks verify:
  ...
  - UART2 device presence (e.g. /dev/ttyAMA4)
...
def step_rtk_uart2_check(api, system_id, result, uart_port=None):
    ...
    Verifies:
      ...
      - UART4 device (e.g. /dev/ttyAMA4) exists on the system
    ...
    uart_port : str or None
        UART4 serial device path (default: /dev/ttyAMA4).
    """
    logger.info("=" * 60)
    logger.info(" Step: RTK_UART2 Check (F9P UART2 Direct Monitoring)")
    logger.info("=" * 60)

    port = uart_port or "/dev/ttyAMA4"
...
def main():
    ...
    parser.add_argument("--rtk-uart-port", default="/dev/ttyAMA4",
                        help="UART4 serial device path for RTK checks "
                             "(default: /dev/ttyAMA4)")
