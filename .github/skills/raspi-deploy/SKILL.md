---
name: raspi-deploy
description: "コードを編集してRaspberry Piでデバッグ・実行するワークフロー。使用するとき: コードに変更を加えた後にRaspberry Pi (taki@raspi5.local) へデプロイして実行・検証したいとき。gh cli でプッシュ → SSH で git pull → 仮想環境(.venv)で実行 → 結果を検証 → エラーがあれば再編集してループというサイクルを自動化する。'デプロイ', 'ラズパイで実行', 'Raspberry Piでテスト', 'raspi deploy', 'deploy and test', 'mavlink-routerを起動' などのフレーズで呼び出される。"
argument-hint: "実行する操作 (例: main.py を起動, mavlink-router を設定, テストを実行)"
---

# raspi-deploy スキル

Raspberry Pi 5 (taki@raspi5.local) へのデプロイ・実行・検証サイクルを自動化するスキルです。

## 対象環境

| 項目 | 値 |
|------|-----|
| Raspberry Pi ホスト (mDNS推奨) | `taki@raspi5.local` |
| 〃 (Wi-Fi Buffalo) | `taki@192.168.11.51` |
| 〃 (テザリング iPhone) | `taki@172.20.10.12` |
| テザリング SSID | `ぶ` |
...
ssh-copy-id taki@raspi5.local

# 方法B: PowerShell ネイティブ（ssh-copy-id がない場合）
$pubkey = Get-Content "$env:USERPROFILE\.ssh\id_ed25519.pub"
ssh taki@raspi5.local "mkdir -p ~/.ssh && chmod 700 ~/.ssh && echo '$pubkey' >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"
# ← ここだけパスワード入力が必要
```
...
```powershell
ssh taki@raspi5.local "echo 'SSH auth OK'"
...
```powershell
# ラズパイ側の SSH 設定確認
ssh taki@raspi5.local "sudo grep -E 'PubkeyAuthentication|AuthorizedKeysFile' /etc/ssh/sshd_config"
# → PubkeyAuthentication yes になっていること

# ラズパイ側のパーミッション確認・修正
ssh taki@raspi5.local "ls -la ~/.ssh/"
ssh taki@raspi5.local "chmod 700 ~/.ssh && chmod 600 ~/.ssh/authorized_keys"

# ラズパイ側 SSH デーモンを再起動
ssh taki@raspi5.local "sudo systemctl restart ssh"

# Windows 側 known_hosts をリセット（IP 変更後などに必要）
ssh-keygen -R raspi5.local
```
...
```powershell
ssh taki@raspi5.local "git clone https://github.com/KeitaTK/GCS-UmemotoLab.git ~/GCS-UmemotoLab"
```

### 1-2. 仮想環境を作成してパッケージをインストール

```powershell
ssh taki@raspi5.local "cd ~/GCS-UmemotoLab && python3 -m venv .venv && .venv/bin/pip install -r requirements.txt"
```
...
```powershell
ssh taki@raspi5.local "sudo apt update && sudo apt install -y mavlink-router"
```

インストール後の動作確認:

```powershell
ssh taki@raspi5.local "mavlink-routerd --version"
```
...
```zsh
ssh taki@raspi5.local "cd ~/GCS-UmemotoLab && git pull"
```
...
```powershell
ssh taki@raspi5.local "cd ~/GCS-UmemotoLab && source .venv/bin/activate && timeout 30 python3 app/main.py 2>&1"
```

#### 3-B: テストを実行

```powershell
ssh taki@raspi5.local "cd ~/GCS-UmemotoLab && source .venv/bin/activate && python3 -m pytest tests/ -v 2>&1"
```
...
```powershell
ssh taki@raspi5.local "mavlink-routerd -e raspi5.local:14550 /dev/ttyAMA0:57600 &"
```

| 引数 | 内容 |
|------|------|
| `-e raspi5.local:14550` | GCS（このラズパイ）へ UDP 転送 |
| `/dev/ttyAMA0:57600` | Pixhawk シリアル接続 (ボーレート要確認) |

mavlink-router の設定ファイルを使う場合:

```powershell
ssh taki@raspi5.local "cat /etc/mavlink-router/main.conf"
```
...
| `Host key verification failed` | known_hosts の古いエントリ | `ssh-keygen -R raspi5.local` を実行 |
| `ModuleNotFoundError` | パッケージ未インストール | `ssh taki@raspi5.local "cd ~/GCS-UmemotoLab && .venv/bin/pip install <pkg>"` |
...
| `git pull` で競合 | ラズパイ側に変更あり | `ssh taki@raspi5.local "cd ~/GCS-UmemotoLab && git stash && git pull"` |
...
エラーが3回以上連続する場合:
1. より根本的な原因を調査する
2. ラズパイ上のログを確認: `ssh taki@raspi5.local "journalctl -n 50"`
3. 仮想環境の再構築:
   ```powershell
   ssh taki@raspi5.local "cd ~/GCS-UmemotoLab && rm -rf .venv && python3 -m venv .venv && .venv/bin/pip install -r requirements.txt"
   ```
