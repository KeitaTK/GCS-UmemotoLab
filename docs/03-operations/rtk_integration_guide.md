...
# RTCM受信確認
ssh taki@100.123.158.105 "tail -f /tmp/backend.log | grep RTCM"
...
```bash
# 1. Ntripサーバー接続確認
ssh taki@100.123.158.105 "nc -zv 192.168.11.19 15000"

# 2. RTCMリーダーログ確認
ssh taki@100.123.158.105 "grep -i rtcm /tmp/backend.log | tail -20"

# 3. Pixhawk シリアル接続確認
ssh taki@100.123.158.105 "cat /dev/ttyACM0 | od -A x -t x1 | head"
# 0xFD で始まる MAVLink フレーム期待

# 4. テストサーバー実行
ssh taki@100.123.158.105 "python3 app/dummy_rtcm_server.py &"
...
ssh taki@100.123.158.105 "python3 app/backend_minimal.py &"
sleep 2

# 2. GPS状態確認
ssh taki@100.123.158.105 "python3 -c \"
...
ssh taki@100.123.158.105 "cd ~/GCS-UmemotoLab && cat > /tmp/rtk_config.json <<EOF
{
  \"rtk_enabled\": true,
...
# 4. ロググ監視
ssh taki@100.123.158.105 "tail -f /tmp/backend.log | grep -E 'RTCM|GPS|RTK'"
```
