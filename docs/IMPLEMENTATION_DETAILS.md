...
| グラフプレースホルダー | グラフ表示予定地 | ❌ 未実装 |
| RTK ステータス | RTK 状態表示 | ⚠️ 初期化のみ |
...
| HEARTBEAT | ✅ | ✅ | ✅ | ドローンリスト表示 |
| SYS_STATUS | ✅ | ✅ | ❌ | 受信のみ |
| GLOBAL_POSITION_INT | ✅ | ✅ | ❌ | 受信のみ |
| NAMED_VALUE_FLOAT | ✅ | ✅ | ✅ | テキスト表示のみ |
| **制御** | | | | |
| Arm/Disarm | ✅ | ✅ | ✅ | ボタン実装 |
| Takeoff | ✅ | ✅ | ✅ | 高度固定 10m |
| Land | ✅ | ✅ | ✅ | パラメータなし |
...
- グラフ表示がプレースホルダーのままで実装未完
- SYS_STATUS（バッテリー）表示がない
- 位置情報（GPS）の地図表示がない
- 複数 NAMED_VALUE_FLOAT の管理未実装

### 2. コマンド実行の不完全さ

- コマンド送信後の `COMMAND_ACK` 確認応答がない
- タイムアウト処理なし
- 複数コマンドキューイング未対応
